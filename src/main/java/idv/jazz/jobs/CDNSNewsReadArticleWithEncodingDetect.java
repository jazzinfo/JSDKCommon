...
public class CDNSNewsReadArticleWithEncodingDetect {
    ...
    private void batchInsertJob(File file) {
       	NewsService service = new NewsService();
        try {
            Charset charset = CommonUtil.detectCharset(file);
            System.out.println("偵測到編碼: " + charset.displayName());

            List<News> newsList = CommonUtil.parseNewsFromFile(file, START_MARK, END_MARK , charset);
